...
export default function PesertaEventCard({ id, judul, lokasi, tanggal, waktu, poster, qrCode }: Props) {
  const now = new Date();
  ...
  return (
    <div className="relative bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition">
      <Link href={`/event/${id}?from=dashboard`} className="block">
        <div className="relative h-32 bg-gradient-to-br from-[#EDF3FF] to-[#DEE7FF] flex items-center justify-center overflow-hidden">
          {hariLagiBadge && (
            <span className="absolute top-2 left-2 text-[11px] font-semibold bg-red-500 text-white px-2.5 py-1 rounded-full z-10">
              {hariLagiBadge}
            </span>
          )}
          {poster ? (
            <Image src={poster} alt={judul} fill className="object-cover" />
          ) : (
      ...
      {qrCode && (
        <div className="absolute top-2 right-2 z-10">
          <QRCodeModal value={qrCode} label={judul} />
        </div>
      )}
    </div>
  );
