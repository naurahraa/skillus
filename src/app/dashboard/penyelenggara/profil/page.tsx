...
import ProfilPenyelenggaraForm from "@/components/dashboard/ProfilPenyelenggaraForm";
...
export default async function ProfilPenyelenggaraPage() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6">
      <ProfilPenyelenggaraForm
        email={user?.email ?? ""}
        namaPic={user?.penyelenggaraProfile?.namaPic ?? ""}
        organisasi={user?.penyelenggaraProfile?.organisasi ?? ""}
        noHp={user?.penyelenggaraProfile?.noHp ?? ""}
      />
