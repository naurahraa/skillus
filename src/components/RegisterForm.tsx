...
export default function RegisterForm() {
  ...
  const initialRole = searchParams.get("role") === "penyelenggara" ? "penyelenggara" : "peserta";
  ...
  function handleRoleChange(newRole: Role) {
    setRole(newRole);
    setForm({ nama: "", email: "", password: "", orgOrInstitusi: "", noHp: "" });
    setError("");
  }
    ...
    const payload =
      role === "peserta"
        ...
        : {
            ...
            role,
            namaPic: form.nama,
            organisasi: form.orgOrInstitusi,
            noHp: form.noHp,
          };
      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-[#EDF3FF] to-white items-center justify-center px-16">
        <div className="max-w-sm flex flex-col items-start gap-6">
          ...
          <h2 className="text-3xl font-bold text-[#1A194D]">
            {isPeserta ? "Gabung sebagai Peserta" : "Gabung sebagai Penyelenggara"}
          </h2>

          <p className="text-lg font-medium text-[#4F4CEE] italic">
            {isPeserta
              ? '"Ikuti berbagai event menarik dan kembangkan potensimu bareng SkillUs!"'
              : '"Buat dan publikasikan event akademikmu dengan cepat, rapi, dan profesional!"'}
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-1">
                {isPeserta ? "Nama Lengkap" : "Nama PIC"}
              </label>
              <input
                type="text"
                required
                ...
                placeholder={isPeserta ? "Masukkan nama lengkap" : "Masukkan nama PIC"}
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-900 mb-1">
                {isPeserta ? "Institusi" : "Organisasi"}
              </label>
              <input
                type="text"
                required
                value={form.orgOrInstitusi}
                onChange={(e) => handleChange("orgOrInstitusi", e.target.value)}
                placeholder={isPeserta ? "Masukkan nama institusi" : "Masukkan nama organisasi"}
                className="w-full border border-[#D4D9E3] rounded-lg px-4 py-3 text-sm placeholder:text-[#8493AC] focus:outline-none focus:ring-2 focus:ring-[#4F4CEE]"
              />
            </div>
            ...
            <p className="text-center text-sm text-gray-600">
              Sudah punya akun?{" "}
              <Link href="/login" className="text-[#4F4CEE] font-medium hover:underline">
                Masuk di sini
              </Link>
            </p>
          </form>
